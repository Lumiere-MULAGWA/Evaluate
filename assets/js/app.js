...
const EvaluationForm = {
    ...
    setupCommentAreas() {
        const commentTextareas = Utils.$$('.comment-textarea');
        ...
        commentTextareas.forEach(textarea => {
            ...
            const updateCounter = () => {
                ...
                if (counterElement) {
                    ...
                    const parentCounter = counterElement.parentNode;
                    if (currentLength > maxLength * 0.9) {
                        parentCounter.style.color = 'var(--danger-color)';
                    } else if (currentLength > maxLength * 0.7) {
                        parentCounter.style.color = 'var(--warning-color)';
                    } else {
                        parentCounter.style.color = 'var(--gray-600)';
                    }
                }
            };
            ...
            // Amélioration de l'UX des commentaires
            textarea.addEventListener('focus', () => {
                Utils.addClass(textarea.parentNode, 'comment-focused');
                
                // Afficher des suggestions si le champ est vide
                if (textarea.value.trim() === '') {
                    this.showCommentSuggestions(textarea);
                }
            });
            
            textarea.addEventListener('blur', () => {
                Utils.removeClass(textarea.parentNode, 'comment-focused');
                this.hideCommentSuggestions(textarea);
            });
    ...
    showCommentSuggestions(textarea) {
        // Supprimer les suggestions existantes
        this.hideCommentSuggestions(textarea);
        
        const suggestions = [
            "Points forts observés dans ce domaine...",
            "Axes d'amélioration recommandés...",
            "Suggestions pour le développement professionnel...",
            "Formation recommandée pour améliorer...",
            "Excellente performance, continuez ainsi..."
        ];
        
        const suggestionsContainer = document.createElement('div');
        suggestionsContainer.className = 'comment-suggestions';
        suggestionsContainer.innerHTML = `
            <div class="suggestions-header">
                <i class="fas fa-lightbulb"></i>
                <span>Suggestions de commentaires:</span>
            </div>
        ...
        // Ajouter les event listeners pour les suggestions
        suggestionsContainer.querySelectorAll('.suggestion-item').forEach(item => {
            item.addEventListener('click', () => {
                ...
                // Masquer les suggestions et donner le focus au textarea
                this.hideCommentSuggestions(textarea);
                textarea.focus();
            });
        });
    ...
    hideCommentSuggestions(textarea) {
        const existingSuggestions = textarea.parentNode.querySelector('.comment-suggestions');
...
const EvaluationManager = {
    ...
    generatePreviewContent() {
        ...
        if (employeeSections.length === 0) {
            return '<div class="preview-no-data"><p><i class="fas fa-info-circle"></i> Aucune évaluation à prévisualiser.</p></div>';
        }
        
        employeeSections.forEach(section => {
            const employeeId = section.dataset.employeeId;
            const employeeHeader = section.querySelector('.employee-header');
            const employeeName = employeeHeader ? employeeHeader.querySelector('h2').textContent : 'Employé inconnu';
            const employeeMeta = employeeHeader ? employeeHeader.querySelector('.employee-meta').textContent.trim() : '';
            
            html += `
                <div class="preview-employee">
                    <div class="preview-employee-header">
                        <h3><i class="fas fa-user"></i> ${employeeName}</h3>
                        <p class="preview-employee-info">${employeeMeta}</p>
                    </div>
            ...
            criteriaSections.forEach(criteriaSection => {
                ...
                const commentTextarea = criteriaSection.querySelector('.comment-textarea');
                ...
                if (selectedRating) {
                    ...
                    html += `
                        <div class="preview-criteria-item">
                            ...
                            ${comment ? `<div class="criteria-comment">
                                <i class="fas fa-comment-alt"></i>
                                <p>${comment}</p>
                            </div>` : '<div class="no-comment"><i class="fas fa-comment-slash text-muted"></i> Aucun commentaire</div>'}
                        </div>
                    `;
                } else {
                    html += `
                        <div class="preview-criteria-item incomplete">
                            <div class="preview-criteria-header">
                                <span class="criteria-name">
                                    <i class="fas fa-exclamation-circle text-warning"></i>
                                    ${criteriaTitle}
                                </span>
                                <span class="criteria-score incomplete">Non évalué</span>
                            </div>
                        </div>
                    `;
                }
            ...
            const completionPercentage = criteriaSections.length > 0 ? ((completedCriteria / criteriaSections.length) * 100).toFixed(0) : 0;
            
            globalTotalScore += totalScore;
            globalCriteriaCount += criteriaCount;
            
            html += `
                    </div>
                    <div class="preview-employee-summary">
                        <div class="summary-stats">
                            <div class="stat-item">
                                <span class="stat-label">Progression:</span>
                                <span class="stat-value">${completedCriteria}/${criteriaSections.length} critères (${completionPercentage}%)</span>
                            </div>
                            <div class="stat-item">
                                <span class="stat-label">Moyenne:</span>
                                <span class="stat-value ${this.getScoreClass(averageScore)}">${averageScore}/20</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        // Résumé global
        const globalAverage = globalCriteriaCount > 0 ? (globalTotalScore / globalCriteriaCount).toFixed(1) : 0;
        
        html += `
            <div class="preview-global-summary">
                <h3><i class="fas fa-chart-bar"></i> Résumé global de l'évaluation</h3>
                <div class="global-stats">
                    <div class="global-stat">
                        <span class="stat-number">${employeeSections.length}</span>
                        <span class="stat-text">Employé(s)</span>
                    </div>
                    <div class="global-stat">
                        <span class="stat-number">${globalCriteriaCount}</span>
                        <span class="stat-text">Critères évalués</span>
                    </div>
                    <div class="global-stat">
                        <span class="stat-number ${this.getScoreClass(globalAverage)}">${globalAverage}/20</span>
                        <span class="stat-text">Moyenne générale</span>
                    </div>
                </div>
            </div>
        `;
